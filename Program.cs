...
namespace TestingCode_1
{
    class Program
    {
        static Random rand = new Random();
        static string[] imagePaths = Directory.GetFiles("art");
        ...
        static void CreateGif(Bitmap image1)
        {
            ...
            GifWriter gif = new GifWriter(String.Format("{0}.gif", DateTime.Now.ToString("MM dd yyyy hh mm ss")), 33, 0);
            gif.DefaultWidth = image1.Width;
            gif.DefaultHeight = image1.Height;

            for (int i = 0; i < 30; i++) // count of unique frames
            {
                for (int w = 0; w < image1.Width; w++)
                {
                    for (int h = 0; h < image1.Height; h++)
                    {
                        Color color = Color.FromArgb(255, image1.GetPixel(w, h));
                        float probability = (float)rand.Next(0, 100) / 100;
                        /*if (color.GetBrightness() < .1f)
                       {
                           //image2.SetPixel(w, h, Color.Black); //enable for high contrast
                       }
                       else if(color.GetBrightness() > .9f)
                       {
                           //image2.SetPixel(w, h, Color.White); //enable for high contrast
                       }
                       else */
                        if (probability <= color.GetBrightness())

                        {
                            image2.SetPixel(w, h, Color.White);
                        }
                        else if (probability > color.GetBrightness())
                        {
                            image2.SetPixel(w, h, Color.Black);
                        }
                    }
                }
                gif.WriteFrame(image2, 33); // 33 miliseconds for 30fps
            }
            Console.WriteLine("Finished: " + image1);
        }
